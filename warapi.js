...
exports.updateMap = function () {
  ...
  try {
    sql.prepare('DROP TABLE apidata_dynamic_temp;').run();
  } catch (err) {
  ...
  try {
    sql
      .prepare(
        'CREATE TABLE apidata_dynamic (regionName TEXT PRIMARY KEY, regionId INT, data TEXT, etag TEXT);'
      )
      .run();
  } catch (err) {}
  var etags = sql
    .prepare('SELECT regionName, etag FROM apidata_dynamic;')
    .all();
  ...
  const insertMap = sql.prepare(
    'INSERT OR REPLACE INTO apidata_dynamic (regionName, regionId, data, etag) VALUES (@regionName, @regionId, @data, @etag);'
  );
  const getMap = sql.prepare(
    'SELECT * FROM apidata_dynamic WHERE regionId = ?;'
  );
  //  Dynamic Data
            ...
            if ((obj.iconType > 3 && obj.iconType < 8) || obj.iconType == 29) {
              ...
              if (JSON.stringify(obj) != JSON.stringify(prevObj)) {
                const insertEvent = sql.prepare(
                  'INSERT INTO events (region, date, prevItem, newItem)  VALUES (@region, @date, @prevItem, @newItem);'
                );
                let entry = {
                ...
                if (
                  ...
                  let globallist = sql
                    .prepare(
                      'SELECT * FROM global WHERE settings LIKE ? AND settings LIKE "%channelid%" ;'
                    )
                    .all('%' + signature + '%');
...
exports.pullStatic = function () {
  ...
  try {
    sql.prepare('DROP TABLE apidata_static_temp;').run();
  } catch (err) {
    //K - if there is no table to drop, just ignore the error and create a new one, meh.
  }
  try {
    sql
      .prepare(
        'CREATE TABLE apidata_static (regionName TEXT PRIMARY KEY, regionId INT, data TEXT, etag TEXT);'
      )
      .run();
  } catch (err) {}
  var etags = sql.prepare('SELECT regionName, etag FROM apidata_static;').all();
  const insertMap = sql.prepare(
    'INSERT OR REPLACE INTO apidata_static (regionName, regionId, data, etag) VALUES (@regionName, @regionId, @data, @etag);'
  );
  //Static Data
  var staticCounter = 0;
  let startLoadDate = new Date();
